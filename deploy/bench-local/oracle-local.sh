...
docker run -d --name ol-surch --network "$NET" --memory=512m --memory-swap=512m \
  ...
  ${SURCH_SOURCE_FETCH_PROFILE:+-e SURCH_SOURCE_FETCH_PROFILE="$SURCH_SOURCE_FETCH_PROFILE"} \
  ${SURCH_SOURCE_FETCH_PROFILE_FILE:+-e SURCH_SOURCE_FETCH_PROFILE_FILE="$SURCH_SOURCE_FETCH_PROFILE_FILE"} \
