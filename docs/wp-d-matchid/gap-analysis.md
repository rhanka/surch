...
| **A6** | `prefix` query + `index_prefixes` mapping option | autocomplete | partial | `crates/surch-api/src/search.rs` (`SearchQuery::Prefix`, `parse_prefix_query`, `prefix_field_matches`, `prefix_token_spans`); `crates/surch-index/src/mapping.rs` (`FieldPrefixes`, `FieldMapping::index_prefixes`, `parse_index_prefixes`, `MappingError::IndexPrefixes*`); 3 new `from_properties_value_*_index_prefixes_*` tests | `prefix` query already shipped against the standard postings scan; this batch parses the field-level `index_prefixes: { min_chars, max_chars }` option (matchID `PRENOMS` shape) so `PUT /:index` replays without HTTP 400. Defaults follow ES (`min=2`, `max=5`); validation enforces `2 <= min < max <= 20` and rejects non-text fields. Runtime acceleration (write-time edge-ngram fan-out into a dedicated `prefix` postings list) is **pending** — today's executor still iterates tokens via `prefix_token_spans`. |
...
| **B1** | matchID replay fixture | acceptance | implemented (partial — 24 tests passing, 6 skipped on gaps A4/A6) | `tests/matchid_compat/replays/deces_v1.json`, `crates/surch-api/tests/matchid_compat.rs` | v1: 30 representative requests (8 advanced + 5 block-match + 4 full-text + 3 sort + 2 function_score + 3 prefix + 3 scroll + 2 range). 24 execute against Surch HEAD; 3 prefix on gap A6 + 3 scroll on gap A4 skipped. Expectations captured against Surch HEAD on 2026-05-15 (`cargo run --release -p surch-api` + curl); ES-7.x cross-check still pending (B1 phase 3, gated on poc-k8s). |
