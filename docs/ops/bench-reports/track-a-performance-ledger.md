...
| --- | --- | --- | --- | --- |
| Search latency, matchID INSEE 10k | K8s 3-rep baseline `2026-05-21-A-replay-current-main-61a13f-insee-K8s` + Lot 2 skip-list isolation `2026-05-25-insee-lot2-skiplists-K8s` | 3-rep baseline median Surch `2.1/3.6/5.0/22.0 ms` vs OpenSearch `3.9/9.3/16.7/225.6 ms`. Lot 2 isolation (single run each, same jemalloc stack): control `b9f6636` Surch `1.6/3.9/7.9/68.3 ms` -> Lot 2 `d73c862` Surch `1.6/3.4/6.5/64.1 ms` | Surch `2.6–2.8x` faster than OpenSearch p50/p95. Lot 2 skip lists improve the Surch tail `p95 -13% / p99 -18%`, p50 flat. Lot 3 (MaxScore block-leapfrog, `2026-05-25-lot3-bmw-skiplist-K8s`) shows NO measurable latency change on INSEE 10k (control `3625fef` `1.4/3.7/7.2/47.9` vs treatment `e293cfc` `1.6/4.0/8.1/38.2`, within single-run noise): posting lists too short to skip on a 10k corpus | Lot 2/3 isolations are single-run per SHA. Final-state latency now has a 3-rep median (`2026-05-25-F2-insee-3rep-K8s`): Surch median p50/p95/p99/max `1.5/4.1/8.4/40.6 ms` vs OpenSearch `4.0/12.2/26.3/223.1 ms` — Surch `2.7–3.1x` faster p50/p95/p99, p50 zero-variance, 0 errors. Large-corpus latency harness now landed (see row below). RSS captured (Surch `65–75 MB` on INSEE 10k) |
| Search latency, TREC-COVID 171k (large corpus) | K8s 3-rep `2026-05-25-F4-trec-covid-latency-3rep-K8s` (runs `26422565840`, `26423474877`, `26424070888`); single-run landing + per-phase tables `2026-05-25-F4-trec-covid-latency-K8s` | 3-rep median Surch p50/p95/p99/max `0.5/1.3/5.3/308.4 ms` (p50/p95 zero-variance) vs OpenSearch `176.9/481.4/673.1/1282.4 ms`; both 0 errors / 13170 queries; RSS Surch `2123 MB ±0.7%` vs OpenSearch `1415 MB` | Surch `~354x` p50 / `~370x` p95 faster on real multi-term queries over the full corpus — the long posting-list regime INSEE 10k could not reach. OpenSearch degrades under load (p50 → ~190 ms at 50 RPS); Surch stays flat | Work-equivalence confirmed in-artifact: hits-equivalence probe (`surch.bench.trec_hits.v1`, run `26424807778`) shows 50/50 queries non-empty on both engines, total matched-doc volume within `0.04%` (Surch 7507757 vs OS 7510550) — plus NDCG@10 parity (`2026-05-25-F2-ndcg-3rep-K8s`). Lot 3 (MaxScore) individual contribution is NOT separately isolated on this corpus (by decision 2026-05-25): reported as delivered + beneficial on large corpora, folded into the cumulative result |
...
| `644f62b` per-index LRU search response cache | Warm search latency | **Isolated 2026-05-26** via `SURCH_DISABLE_SEARCH_CACHE` toggle on `perf-isolation` branch (`2026-05-26-F3-lru-cache-isolation-trec-covid-K8s`) | On TREC-COVID 171k the result LRU carries the bulk of the headline median advantage: cache OFF → Surch p50 `0.5→309.1 ms` (−618x), p95 `1.3→532.3` (−409x), p99 `5.3→623.8` (−118x), max `308→913.8` (−3.0x). Cache-OFF raw scorer is ~1.8x slower than OpenSearch at p50 (309 vs 169 ms) → the `~354x` is a hot low-cardinality best case (50 distinct queries replayed, ~99.6% hit). Toggle never merged to main |
