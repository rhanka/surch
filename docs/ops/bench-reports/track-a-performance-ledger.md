...
| RSS / memory | `docs/ops/memory-capacity.md` + K8s pod limits + K8s `ndcg-gate` runs `26340177506` (`137b352`, full-rebuild baseline), `26350556060` (`04fde72`, incremental bulk), `26359069219` (`01ad77e`, Lot 1.5 RAM) | Capacity model: BAN 25k ~85 MB, INSEE 1.3M ~4.5 GB. K8s BEIR full 171 k TREC-COVID paired sampling (1 Hz, 1200 s): Surch peak `4802 MiB` (full-rebuild) -> `5859 MiB` (incremental Lot 1) -> `5591 MiB` (Lot 1.5 refresh-finalize). OpenSearch peak essentially unchanged `1395 / 1459 / 1461 MiB`. Lot 1.5 logical free works (test covers post-refresh rebuild) but recovers only `268 MiB` system RSS because glibc default allocator does not return freed pages without memory pressure | Surch fits under the 7 GiB cap on all paths. Full `~1 GiB` allocator gain requires `malloc_trim(0)` after `finalize_postings()` or a jemalloc switch (tracked as Lot 1.7) | INSEE-side replay still needs a paired RSS rerun |
