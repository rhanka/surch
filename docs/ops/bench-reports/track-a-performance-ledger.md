...
| Bulk indexing | Local report `2026-05-16-vs-os-2.17.1` plus K8s `ndcg-gate` runs `26304471549` (`d9cac15`, full-rebuild baseline), `26340177506` (`137b352`, paired RSS), and `26350556060` (`04fde72`, incremental bulk Lot 1 fix) | Local SciFact: Surch `3.545 s`, OpenSearch `17.612 s`; local BAN 25k: Surch `17.882 s`, OpenSearch `21.707 s`; K8s SciFact across runs: Surch `2.29–3.66 s`, OpenSearch `7.84–11.38 s` (Surch `2.1–4.8x` faster); K8s TREC-COVID `d9cac15`/`137b352` (pre-fix): Surch `1001.95 / 1112.52 s`, OpenSearch `72.27 / 93.80 s` (OpenSearch `11.9–13.9x` faster); K8s TREC-COVID `04fde72` (incremental fix): Surch `179.86 s`, OpenSearch `87.04 s` (OpenSearch `2.06x` faster) | Surch wins SciFact bulk consistently. TREC-COVID Surch/OpenSearch bulk gap closed from `13.9x slower` to `2.06x slower` (`~5.6x` speedup) by the incremental append path; remaining gap is the cumulative term-dictionary rebuild on each `_bulk` POST (next candidate) | Lot 1 closed by `04fde72`; remaining bulk gap tracked as a follow-up Lot |
...
| RSS / memory | `docs/ops/memory-capacity.md` + K8s pod limits + K8s `ndcg-gate` runs `26340177506` (`137b352`, full-rebuild baseline) and `26350556060` (`04fde72`, incremental bulk) | Capacity model: BAN 25k ~85 MB, INSEE 1.3M ~4.5 GB. K8s BEIR full 171 k TREC-COVID paired sampling (1 Hz, 1200 s): Surch peak `4802 MiB / 7 GiB` (70 %) on full-rebuild path, `5859 MiB / 7 GiB` (84 %) on incremental path (+`1057 MiB` for the live `PostingsBuilder` snapshot needed by `append_to_index`). OpenSearch peak `1395 MiB / 2 GiB` (70 %) -> `1459 MiB / 2 GiB` (71 %); essentially unchanged | Surch fits under the 7 GiB cap on both paths; the incremental bulk fix trades `~1 GiB` of RAM for `~5.6x` bulk speedup on long-text corpora — Surch footprint is now `~4.0x` OpenSearch peak | Free the live `PostingsBuilder` snapshot after `_refresh` (with rebuild safety on the next write) to recover the `~1 GiB` overhead; INSEE-side replay still needs a paired RSS rerun |
