...
impl FuzzyAlgorithm {
    pub fn damerau_levenshtein(s1: &str, s2: &str, max_distance: usize) -> usize {
        ...
        if (len1 as usize) > max_distance * 2 || (len2 as usize) > max_distance * 2 {
        ...
        for i in 1..=len1 {
            ...
            for j in 1..=len2 {
                ...
                if s1_chars[i - 1] == s2_chars[j - 2] && s2_chars[j - 1] == s1_chars[i - 2] {
                    matrix[i][j] = std::cmp::min(matrix[i][j], prev_diag + 1);
                }
...
mod tests {
    ...
    fn test_damerau_levenshtein() {
        assert_eq!(FuzzyAlgorithm::damerau_levenshtein("ab", "ba", 2), 1);
        assert_eq!(FuzzyAlgorithm::damerau_levenshtein("ab", "abc", 2), 1);
        assert_eq!(FuzzyAlgorithm::damerau_levenshtein("ca", "abc", 2), 2);
    }

    #[test]
    fn test_fuzzy_match() {
        assert!(FuzzyAlgorithm::is_fuzzy_match("hello", "hallo", 1));
        assert!(FuzzyAlgorithm::is_fuzzy_match("hello", "hallo", 2));
        assert!(!FuzzyAlgorithm::is_fuzzy_match("hello", "world", 2));
    }

    #[test]
    fn test_find_matches() {
        let candidates = vec!["hello", "hallo", "hullo", "world", "hero"];
        let matches = FuzzyAlgorithm::find_fuzzy_matches("hello", &candidates, 1);
